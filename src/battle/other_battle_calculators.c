...
void LONG_CALL BattleMessage_ExpandPlaceholders(struct BattleSystem *battleSystem, MsgData *data, BattleMessage *msg) {
    ...
    for (int i = 0; i < battleSystem->msgBuffer->size; i++) {
        if (battleSystem->msgBuffer->data[i] <= 0x0144) {
            character = battleSystem->msgBuffer->data[i] - 234;
        } else {
            switch (battleSystem->msgBuffer->data[i]) {
                case 0xFFFF:
                    character = '\0';
                    break;
                case 0x01BE:
                ...
                case 0x01B3:
                    ...
                    break;
                default:
                    character = battleSystem->msgBuffer->data[i] - 228;
                    break;
            }
        }
        if (character == '’') {
            debug_printf("’");  // screw it, not using my brain for now
        } else {
